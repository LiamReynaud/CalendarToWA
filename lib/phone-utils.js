/**
 * Détection et normalisation de numéros de téléphone (FR + international).
 * Exposé sur window.CalendarToWA pour les content scripts.
 */
(function () {
  const PHONE_PATTERNS = [
    /\+(?:\d[\s.\-]?){8,14}\d/g,
    /\b0[1-9](?:[\s.\-]?\d{2}){4}\b/g,
    /\b0[1-9]\d{8}\b/g,
  ];

  const DIAL_IN_LINE_PATTERNS = [
    /rejoindre\s+par\s+t[ée]l[ée]phone/i,
    /participer\s+par\s+t[ée]l[ée]phone/i,
    /join\s+by\s+phone/i,
    /dial[- ]?in/i,
    /composer\s+(le\s+|un\s+)?[\d\s]/i,
    /\(\s*(FR|US|BE|UK|DE|CH)\s*\)\s*\+?\d/i,
    /pin\s*[:#]\s*\d/i,
    /#\s*\d[\d\s]{3,}#?$/,
  ];

  const DIAL_IN_PREVIOUS_LINE = [
    /^rejoindre\s+par\s+t[ée]l[ée]phone/i,
    /^join\s+by\s+phone/i,
    /^num[ée]ro\s+(de\s+)?t[ée]l[ée]phone/i,
    /^phone\s+numbers?$/i,
  ];

  function getLineAt(text, index, length) {
    const lineStart = text.lastIndexOf("\n", index - 1) + 1;
    const lineEnd = text.indexOf("\n", index + length);
    return text.slice(lineStart, lineEnd === -1 ? text.length : lineEnd);
  }

  function getPreviousLine(text, index) {
    const lineStart = text.lastIndexOf("\n", index - 1) + 1;
    if (lineStart <= 0) return "";
    const prevEnd = lineStart - 1;
    const prevStart = text.lastIndexOf("\n", prevEnd - 1) + 1;
    return text.slice(prevStart, prevEnd);
  }

  function countPhonesInText(text) {
    let count = 0;
    for (const source of PHONE_PATTERNS) {
      const pattern = new RegExp(source.source, source.flags);
      const matches = text.match(pattern);
      if (matches) count += matches.length;
    }
    return count;
  }

  function isContactPhoneLine(line) {
    const trimmed = line.trim();
    if (/^contact:\s*.+,\s*phone:\s*\d/i.test(trimmed)) return true;
    if (/^phone\s*number\s*:/i.test(trimmed)) return true;
    return (
      (/^(num[ée]ro|t[ée]l[ée]phone|phone|mobile|tel\.?|whatsapp|contact)\s*:+/i.test(
        trimmed
      ) ||
        /n[°º]?\s*(de\s+)?t[ée]l[ée]phone\s*:/i.test(trimmed)) &&
      /\d/.test(trimmed)
    );
  }

  function isLabeledContactPhone(text, index) {
    const start = Math.max(0, index - 48);
    const before = text.slice(start, index);
    return /(?:Phone\s*Number|Num[ée]ro(?:\s+de\s+t[ée]l[ée]phone)?|N°\s*de\s+t[ée]l[ée]phone|T[ée]l[ée]phone|Mobile|Tel\.?)\s*:+\s*$/i.test(
      before
    );
  }

  const LABELED_PHONE_PATTERNS = [
    /Phone\s*Number\s*:\s*(\+?\d{9,15})/gi,
    /Num[ée]ro(?:\s+de\s+t[ée]l[ée]phone)?\s*::?\s*(\+?[\d\s.\-]{10,22})/gi,
    /N°\s*de\s*t[ée]l[ée]phone\s*:\s*(\+?[\d\s.\-]{10,22})/gi,
    /T[ée]l[ée]phone\s*:\s*(\+?[\d\s.\-]{10,22})/gi,
  ];

  function extractLabeledPhones(text) {
    const found = new Set();
    if (!text || typeof text !== "string") return found;

    for (const pattern of LABELED_PHONE_PATTERNS) {
      pattern.lastIndex = 0;
      let match;
      while ((match = pattern.exec(text)) !== null) {
        const raw = trimPhoneRaw(match[1].trim());
        const digits = raw.replace(/\D/g, "");
        if (digits.length < 9 || digits.length > 15) continue;
        found.add(raw);
      }
    }

    return found;
  }

  function normalizePipedrivePhoneRaw(raw) {
    const trimmed = trimPhoneRaw((raw || "").trim());
    if (!trimmed) return null;
    const digits = trimmed.replace(/\D/g, "");
    if (digits.length < 9 || digits.length > 15) return null;
    if (trimmed.startsWith("+") || /^[\d\s.\-+]+$/.test(trimmed)) {
      return trimmed.replace(/\s+/g, " ").trim();
    }
    return digits;
  }

  const PIPEDRIVE_NEXT_LABEL = /\s*(?:Participants|Contact|Phone)\s*:/i;

  function trimBeforeNextPipedriveLabel(value) {
    const trimmed = (value || "").trim();
    if (!trimmed) return "";
    const cut = trimmed.search(PIPEDRIVE_NEXT_LABEL);
    return cut > 0 ? trimmed.slice(0, cut).trim() : trimmed;
  }

  /** RDV synchronisés depuis Pipedrive (Deal / Contact / Phone). */
  function extractPipedriveDealName(text) {
    if (!text || typeof text !== "string") return "";
    const match = text.match(
      /(?:^|\n)\s*Deal:\s*(.+?)(?=\s*(?:Participants|Contact|Phone)\s*:|\n|$)/i
    );
    if (!match) return "";
    return trimBeforeNextPipedriveLabel(match[1]);
  }

  function extractPipedriveContacts(text) {
    if (!text || typeof text !== "string") return [];

    const dealName = extractPipedriveDealName(text);
    const contacts = [];
    const seen = new Set();

    function pushContact(nameFromContact, phoneRaw) {
      const contactName = trimBeforeNextPipedriveLabel(nameFromContact);
      const phone = normalizePipedrivePhoneRaw(phoneRaw);
      const displayName = contactName || dealName;
      if (!displayName && !phone) return;
      const key = `${contactName.toLowerCase()}|${phone || ""}`;
      if (seen.has(key)) return;
      seen.add(key);
      contacts.push({
        dealName,
        contactName,
        displayName,
        phoneRaw: phone,
      });
    }

    const inlineRe =
      /Contact:\s*([^,]+)(?:,\s*Phone\s*:\s*([^,\n]{5,30}))?/gi;
    let match;
    while ((match = inlineRe.exec(text)) !== null) {
      pushContact(match[1], match[2]);
    }

    if (!contacts.some((c) => c.phoneRaw)) {
      const loosePhoneRe = /(?<![A-Za-zÀ-ÿ])Phone\s*:\s*([^,\n;]{5,30})/gi;
      while ((match = loosePhoneRe.exec(text)) !== null) {
        const phone = normalizePipedrivePhoneRaw(match[1]);
        if (!phone) continue;
        const existing = contacts.find((c) => c.phoneRaw === phone);
        if (existing) continue;
        const orphan = contacts.find((c) => !c.phoneRaw);
        if (orphan) orphan.phoneRaw = phone;
        else pushContact("", phone);
      }
    }

    if (dealName && contacts.length === 0) {
      pushContact("", null);
      contacts[contacts.length - 1].displayName = dealName;
    }

    return contacts;
  }

  function isFrenchLandline(rawPhone, defaultCountryCode = "33") {
    if (defaultCountryCode !== "33") return false;
    const digits = rawPhone.replace(/\D/g, "");
    let national = digits;
    if (digits.startsWith("0033")) national = "0" + digits.slice(4);
    else if (digits.startsWith("33") && digits.length >= 11)
      national = "0" + digits.slice(2);
    return /^0[1-59]\d{8}$/.test(national);
  }

  function isMeetingDialInContext(text, index, length) {
    if (isLabeledContactPhone(text, index)) return false;

    const line = getLineAt(text, index, length);
    const prevLine = getPreviousLine(text, index);

    if (isContactPhoneLine(line)) return false;

    if (DIAL_IN_LINE_PATTERNS.some((re) => re.test(line))) return true;

    if (DIAL_IN_PREVIOUS_LINE.some((re) => re.test(prevLine.trim()))) return true;

    if (countPhonesInText(line) >= 2 && !isLabeledContactPhone(text, index)) return true;

    return false;
  }

  function extractPhones(text, options = {}) {
    const { excludeMeetingDialIn = true, defaultCountryCode = "33" } = options;
    if (!text || typeof text !== "string") return [];

    const found = new Set();

    for (const labeled of extractLabeledPhones(text)) {
      found.add(labeled);
    }

    for (const pattern of PHONE_PATTERNS) {
      pattern.lastIndex = 0;
      let match;
      while ((match = pattern.exec(text)) !== null) {
        const raw = trimPhoneRaw(match[0].trim());
        const digits = raw.replace(/\D/g, "");
        if (digits.length < 9 || digits.length > 15) continue;

        const line = getLineAt(text, match.index, match[0].length);

        if (excludeMeetingDialIn) {
          if (isLabeledContactPhone(text, match.index)) {
            found.add(raw);
            continue;
          }
          if (isMeetingDialInContext(text, match.index, match[0].length)) continue;
          if (isFrenchLandline(raw, defaultCountryCode) && !isContactPhoneLine(line))
            continue;
        }

        found.add(raw);
      }
    }

    for (const { phoneRaw } of extractPipedriveContacts(text)) {
      if (phoneRaw) found.add(phoneRaw);
    }

    return [...found];
  }

  function normalizeForWhatsApp(rawPhone, defaultCountryCode = "33") {
    let digits = rawPhone.replace(/\D/g, "");
    if (!digits) return null;

    if (digits.startsWith("00")) {
      digits = digits.slice(2);
    } else if (digits.startsWith("0") && digits.length === 10) {
      digits = defaultCountryCode + digits.slice(1);
    } else if (digits.length === 9 && /^[67]/.test(digits)) {
      digits = defaultCountryCode + digits;
    }

    if (digits.length < 10 || digits.length > 15) return null;
    return digits;
  }

  function formatDisplay(rawPhone) {
    const digits = rawPhone.replace(/\D/g, "");
    if (digits.length === 10 && digits.startsWith("0")) {
      return digits.replace(/(\d{2})(?=\d)/g, "$1 ").trim();
    }
    if (rawPhone.startsWith("+")) return rawPhone;
    return "+" + digits;
  }

  const COUNTRY_CODES = [
    "998", "996", "995", "994", "993", "992", "977", "976", "975", "974",
    "973", "972", "971", "970", "968", "967", "966", "965", "964", "963",
    "962", "961", "960", "886", "880", "856", "855", "853", "852", "850",
    "692", "691", "690", "689", "688", "687", "686", "685", "683", "682",
    "681", "680", "679", "678", "677", "676", "675", "674", "673", "672",
    "670", "599", "598", "597", "596", "595", "594", "593", "592", "591",
    "590", "509", "508", "507", "506", "505", "504", "503", "502", "501",
    "423", "421", "420", "389", "387", "386", "385", "383", "382", "381",
    "380", "378", "377", "376", "375", "374", "373", "372", "371", "370",
    "359", "358", "357", "356", "355", "354", "353", "352", "351", "49",
    "48", "47", "46", "45", "44", "43", "41", "40", "39", "36", "34", "33",
    "32", "31", "30", "27", "20", "7", "1",
  ];

  function parsePhoneParts(normalized, defaultCountryCode = "33") {
    const digits = String(normalized).replace(/\D/g, "");
    if (!digits) return { countryCode: defaultCountryCode, nationalNumber: "" };

    for (const code of COUNTRY_CODES) {
      if (digits.startsWith(code)) {
        const national = digits.slice(code.length);
        if (national.length >= 6 && national.length <= 12) {
          return { countryCode: code, nationalNumber: national };
        }
      }
    }

    if (digits.startsWith(defaultCountryCode)) {
      return {
        countryCode: defaultCountryCode,
        nationalNumber: digits.slice(defaultCountryCode.length),
      };
    }

    return { countryCode: defaultCountryCode, nationalNumber: digits };
  }

  const SKIP_NAME_LINE =
    /^(organisateur|organizer|invités?|\d+\s+(invités?|oui|en attente)|nom d['']?événement|lieu:|http|www\.|@|\d+$|prendre des notes|participer|créer un|définir votre|alimenté par|annuler:|replanifier:|synchroniser)/i;

  function looksLikePersonName(str) {
    const s = str.trim();
    if (s.length < 3 || s.length > 50) return false;
    if (SKIP_NAME_LINE.test(s)) return false;
    if (/https?:\/\//.test(s) || /@/.test(s)) return false;
    if (/^\d/.test(s)) return false;
    if (!/[A-Za-zÀ-ÿ]/.test(s)) return false;
    if (/^(lundi|mardi|mercredi|jeudi|vendredi|samedi|dimanche)/i.test(s))
      return false;
    return /^[A-Za-zÀ-ÿ][A-Za-zÀ-ÿ.\-'\s]{0,48}[A-Za-zÀ-ÿ.]?$/.test(s);
  }

  function splitCompoundTitle(title) {
    if (!title) return [];
    return title
      .split(/\s+et\s+|\s*,\s*|\s+\/\s+|\s+&\s+/i)
      .map((s) => s.trim())
      .filter(looksLikePersonName);
  }

  function stripEmojis(str) {
    return str
      .replace(/\p{Extended_Pictographic}/gu, "")
      .replace(/\s+/g, " ")
      .trim();
  }

  function normalizeEventTitle(title) {
    return stripEmojis(title || "")
      .replace(/\s*\((?:copy|copie)\)\s*$/i, "")
      .trim();
  }

  function trimPhoneRaw(raw) {
    const trimmed = (raw || "").trim();
    if (!trimmed) return "";
    const cut = trimmed.split(
      /\s+(?:First Name|Last Name|Email|E-?mail|T[ée]l[ée]phone|Phone|Invit[ée]|Contact|Deal|Status:|http|www\.|@|\|)/i
    )[0];
    return cut.trim();
  }

  const MEETING_TITLE_PREFIX =
    /^(réunion|reunion|meeting|appel|visio|call|sync|rdv|rendez-vous|entretien)$/i;

  const ENTRETIEN_TITLE =
    /^entretien\s+([A-Za-zÀ-ÿ0-9][A-Za-zÀ-ÿ0-9.\-'\s]{0,30}?)\s+[-–—]\s+(.+)$/i;

  function parseEntretienTitle(title) {
    const cleaned = normalizeEventTitle(title);
    const match = cleaned.match(ENTRETIEN_TITLE);
    if (!match) return null;
    const suffix = match[1].trim();
    const name = normalizeEventTitle(match[2].trim());
    if (!suffix || !name || !looksLikePersonName(name)) return null;
    return { suffix, name };
  }

  function escapeRegExp(str) {
    return String(str).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  }

  function extractSuffixFromEvent(title, text, knownSuffixes = []) {
    const entretien = parseEntretienTitle(title);
    if (entretien?.suffix) return entretien.suffix;

    const combined = `${title || ""}\n${text || ""}`;

    const bracket = combined.match(/\[([A-Za-zÀ-ÿ0-9][A-Za-zÀ-ÿ0-9.\-'\s]{1,30})\]/);
    if (bracket) return bracket[1].trim();

    for (const suffix of knownSuffixes) {
      const s = (suffix || "").trim();
      if (!s) continue;
      const re = new RegExp(`\\b${escapeRegExp(s)}\\b`, "i");
      if (re.test(combined)) return s;
    }

    if (/\b(?:équipe\s+)?Alegria\b/i.test(combined)) return "Alegria";
    if (/\bYacht\s+Broker\s+School\b/i.test(combined)) return "YBS";
    if (/\bYBS\b/.test(combined)) return "YBS";
    if (/\bSKL\b/.test(combined)) return "SKL";

    return "";
  }

  function extractFormNameFromText(text) {
    if (!text || typeof text !== "string") return null;

    const firstMatch = text.match(
      /First Name:\s*([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ.\-']{0,30}?)(?=Last Name:|Email|Phone|Quelle|Depuis|$)/i
    );
    const lastMatch = text.match(
      /Last Name:\s*([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ.\-']{0,40}?)(?=Quelle|Depuis|Email|Phone|First Name:|Validation|$)/i
    );

    const first = firstMatch?.[1]?.trim() || "";
    const last = lastMatch?.[1]?.trim() || "";
    if (first && last) return `${first} ${last}`;
    if (first) return first;
    if (last) return last;
    return null;
  }

  function extractIClosedGuestName(title) {
    const cleaned = normalizeEventTitle(title);
    if (!cleaned) return null;

    const dated = cleaned.match(/^(.+?)\s+x\s+.+\s+on\s+\d{1,2}\s+[A-Za-zÀ-ÿ]{3,12}\s+\d{4}/i);
    if (dated) {
      const name = dated[1].trim();
      if (looksLikePersonName(name)) return name;
    }

    if (/\s+x\s+.+\s+(?:on\s+\d|with\s+)/i.test(cleaned)) {
      const guest = cleaned.match(/^(.+?)\s+x\s+/i)?.[1]?.trim();
      if (guest && looksLikePersonName(guest)) return guest;
    }

    return null;
  }

  function extractNamesFromTitle(title) {
    if (!title) return [];
    const cleaned = normalizeEventTitle(title);

    const entretien = parseEntretienTitle(cleaned);
    if (entretien) return [entretien.name];

    const iclosedGuest = extractIClosedGuestName(cleaned);
    if (iclosedGuest) return [iclosedGuest];

    const etOrganizer = cleaned.match(/^(.+?)\s+et\s+(.+)$/i);
    if (etOrganizer) {
      const guest = etOrganizer[1].trim();
      const tail = etOrganizer[2].trim();
      if (
        looksLikePersonName(guest) &&
        /^(?:Renaud\s+)?(?:Alegria|YBS|Yacht\s+Broker\s+School)\b/i.test(tail)
      ) {
        return [guest];
      }
    }

    const colonParts = cleaned.match(/^([^:]+):\s*(.+)$/);
    if (colonParts) {
      const prefix = colonParts[1].trim();
      if (looksLikePersonName(prefix) && !MEETING_TITLE_PREFIX.test(prefix)) {
        const fromPrefix = splitCompoundTitle(prefix);
        if (fromPrefix.length) return fromPrefix;
      }
    }

    const dashParts = cleaned.match(/^(.+?)\s+[-–—]\s+(.+)$/);
    if (dashParts) {
      const prefix = dashParts[1].trim();
      if (looksLikePersonName(prefix) && !MEETING_TITLE_PREFIX.test(prefix)) {
        const fromPrefix = splitCompoundTitle(prefix);
        if (fromPrefix.length) return fromPrefix;
      }
    }

    return splitCompoundTitle(cleaned);
  }

  function extractNamesFromText(text) {
    const names = new Set();

    const formName = extractFormNameFromText(text);
    if (formName && looksLikePersonName(formName)) names.add(formName);

    const attendeeMatch = text.match(
      /^([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ.\-'\s]{1,48}?)\s+-\s+\S+@\S+/m
    );
    if (attendeeMatch && looksLikePersonName(attendeeMatch[1].trim())) {
      names.add(attendeeMatch[1].trim());
    }

    const lines = text.split("\n");
    let inGuestSection = false;

    for (const line of lines) {
      const t = line.trim();
      if (!t) continue;

      const inviteLabel = t.match(/^invité(e)?(?:\s*:\s*(.+))?$/i);
      if (inviteLabel) {
        inGuestSection = true;
        const inlineName = (inviteLabel[2] || "").trim();
        if (inlineName && looksLikePersonName(inlineName)) names.add(inlineName);
        continue;
      }

      if (/\d+\s+invités?/i.test(t)) {
        inGuestSection = true;
        continue;
      }

      if (inGuestSection) {
        if (
          /^(lieu:|nom d['']?événement|http|participer|prendre|num[ée]ro|peux-tu|qu['']|email de l['']invité|e-?mail)/i.test(
            t
          )
        )
          break;
        if (/^\d+\s+(oui|en attente)/i.test(t)) continue;
        if (/^(organisateur|organizer)$/i.test(t)) continue;
        if (looksLikePersonName(t)) names.add(t);
      }
    }

    return [...names];
  }

  function extractAttendeeNames(title, text) {
    const names = [];
    const seen = new Set();

    function addName(name) {
      const n = (name || "").trim();
      if (!n || seen.has(n)) return;
      if (
        looksLikePersonName(n) ||
        /^[A-Za-zÀ-ÿ][A-Za-zÀ-ÿ.\-'\s]{1,48}$/.test(n)
      ) {
        seen.add(n);
        names.push(n);
      }
    }

    for (const { contactName, displayName } of extractPipedriveContacts(text)) {
      addName(contactName || displayName);
    }

    extractNamesFromTitle(title).forEach(addName);
    extractNamesFromText(text).forEach(addName);
    return names;
  }

  function formatSuffixForWhatsApp(suffix) {
    const s = (suffix || "").trim();
    if (!s) return "";
    if (s.startsWith("[") && s.endsWith("]")) return s;
    return `[${s}]`;
  }

  window.CalendarToWA = {
    extractPhones,
    normalizeForWhatsApp,
    formatDisplay,
    parsePhoneParts,
    extractAttendeeNames,
    extractSuffixFromEvent,
    parseEntretienTitle,
    extractPipedriveContacts,
    extractPipedriveDealName,
    formatSuffixForWhatsApp,
    isMeetingDialInContext,
    isFrenchLandline,
    isContactPhoneLine,
  };
})();
